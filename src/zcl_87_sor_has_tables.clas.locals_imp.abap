...
DATA standard_table TYPE STANDARD TABLE OF zta_04_flights WITH NON-UNIQUE KEY carrier_id connection_id flight_date.
DATA sorted_table TYPE SORTED TABLE OF zta_04_flights WITH NON-UNIQUE KEY carrier_id connection_id flight_date.
DATA hashed_table TYPE HASHED TABLE OF zta_04_flights WITH UNIQUE KEY carrier_id connection_id flight_date.
...
METHOD constructor.
SELECT FROM zta_04_flights FIELDS * INTO TABLE @standard_table.
SELECT FROM zta_04_flights FIELDS * INTO TABLE @sorted_table.
SELECT FROM zta_04_flights FIELDS * INTO TABLE @hashed_table.
